...
object EcrPlugin extends AutoPlugin {
  ...
  object autoImport {
      ...
      lazy val registroyIds                   = settingKey[Seq[String]]("AWS account IDs that correspond to the Amazon ECR registries.")
  ...
  lazy val tasks: Seq[Def.Setting[_]] = Seq(
    ...
    login := {
      implicit val logger = streams.value.log
      val (user, pass) = AwsEcr.dockerCredentials(region.value, registroyIds.value)
